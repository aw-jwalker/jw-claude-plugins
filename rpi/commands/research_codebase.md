...
3. **Spawn parallel sub-agent tasks for comprehensive research:**
   ...
   **For Linear tickets (if relevant):**
   - Use the **linear-ticket-reader** agent to get full details of a specific
     ticket
   - Use the **linear-searcher** agent to find related tickets or historical
     context
...
5. **Gather metadata for the research document:**
   - Run `thoughts metadata` to generate all relevant metadata (dates, git info,
     researcher name)
   - Determine the filepath for your research document:
     - **Path pattern**:
       `thoughts/shared/research/YYYY-MM-DD-{ticket}-description.md`
     - **Important**: This path is relative to your CURRENT working directory
       (the project repo), NOT the thoughts repo root
     - The `thoughts/` directory in your current repo is a symlink to the
       correct location in the central thoughts repo
     - When using the Write tool, construct the full absolute path:
       `<current_working_directory>/thoughts/shared/research/...`
     - **Path components**:
     ...
     - **Examples** (full paths relative to current repo):
       - With ticket:
         `thoughts/shared/research/2025-01-08-PROJ-1234-parent-child-tracking.md`
       - Without ticket:
         `thoughts/shared/research/2025-01-08-authentication-flow.md`

6. **Generate research document:**
   - Using the Write tool, create your research document at the filepath
     determined in step 5
   - **Important**: Use the full absolute path starting from your current
     working directory
   - Example:
     `/home/user/repos/project-name/thoughts/shared/research/2025-01-08-authentication-flow.md`
   ...
   - Structure the document with YAML frontmatter followed by content:
     ...
     **Date**: [Current date and time with timezone from step 4] **Researcher**:
     [Researcher name from thoughts status] **Git Commit**: [Current commit hash
     from step 4] **Branch**: [Current branch name from step 4] **Repository**:
     [Repository name]
     ...
     [Current patterns, conventions, and design implementations found in the
     codebase]
     ...
     - `thoughts/local/notes.md` - Past exploration of Y Note: Paths exclude
       "searchable/" even if found there
...
8. **Sync and present findings:**
   - After writing the research document, run `thoughts sync` to:
...
9. **Handle follow-up questions:**
   ...
   - Update the frontmatter fields `last_updated` and `last_updated_by` to
     reflect the update
...
- **Path handling**: The thoughts/searchable/ directory contains hard links for
  searching
